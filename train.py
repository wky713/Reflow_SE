...
if __name__ == '__main__':
     ...
     for parser_ in (base_parser, parser):
          ...
          parser_.add_argument("--resume_from", type=str, default=None, 
                            help="Path to checkpoint file to resume training from")  # 新增行
     ...
     # 训练：如果指定了checkpoint路径，则从该路径续训
     if args.resume_from and os.path.exists(args.resume_from):
        print(f"从 checkpoint 续训: {args.resume_from}")
        trainer.fit(model, ckpt_path=args.resume_from)  # 关键修改：添加ckpt_path参数
     else:
          if args.resume_from:
               print(f"警告:指定的checkpoint路径不存在 {args.resume_from}，将从头开始训练")
